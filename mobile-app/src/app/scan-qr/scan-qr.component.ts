...
export class ScanQrComponent implements OnInit {
  ...
  currentDate = this.pipe.transform(Date.now(), 'yyyy-mm-dd hh:mm:ss');
  ...
  ngAfterViewInit(): void {
    ...
    this.qrScannerComponent.capturedQr.subscribe((result: string) => {
      ...
      switch (statut) {
        case 'place':
          //statut: 'place', id:'4', name:'test', description:'le lieu du test 15'
          ...
          this.visit.citizen_id = this.uuid_citizen !== null ? this.uuid_citizen : '' ;
          this.visit.entrance_date = this.currentDate !== null ? this.currentDate.toString() : '';
          console.log(this.visit)

          this.apiService.contact(this.uuid_citizen, this.visit.place_id,this.visit.entrance_date).subscribe(
            (data) => {
              alert("Qr code boum visit")
              alert('QR code scanné le '+ this.visit.entrance_date + ': \nNam : ' + this.visit.name+'\ndescription : '+ this.visit.description  );
            },
            (error) => {
          ...
          break;
        case 'covid':
         // statut: 'covid', id:'6', date: '05/12/2020 14:58:59'  
          //parsing qr code 
         console.log("je rentre covid")
         this.covid.citizen_id =   this.uuid_citizen !== null ? this.uuid_citizen : '' ;
         this.covid.medecin_id =  results[3]
         this.covid.sick_since = results[5]
         console.log(this.covid)
          this.apiService.covid(this.uuid_citizen, this.covid.sick_since).subscribe(
            (data) => {
